...
from seed_ai.types._event import DoneEvent, TextDeltaEvent, ThinkingDeltaEvent  # noqa: E402
...
输出格式:
[
{"dialog":"剧本片段","role":"配音角色","gender":"男或女","tts_control":"TTS语气控制说明"}
]
...
3. 每条只能包含 dialog、role、gender、tts_control 四个字段，不得增加其他字段。
4. dialog 是用于有声书朗读的剧本内容，必须基于原文切分生成。
5. gender 表示该配音角色的性别，只能填写“男”或“女”，不得填写其他值。
6. 旁白的 gender 也必须填写“男”或“女”，并且全文保持一致。
7. 如果角色性别可由原文、姓名、称谓或上下文判断，应按实际角色性别填写“男”或“女”。
8. 如果角色性别无法明确判断，仍必须根据最稳定的角色称谓、文本语境和配音需要选择“男”或“女”，不得留空，不得写“未知”“不明”“其他”等。
9. 全文只能有一个旁白角色，名称必须固定为“旁白”。
10. 所有非对白、非心理独白的叙述性文字，包括环境描写、动作描写、神态描写、场景描写、氛围描写、人物外貌、时间推进、事件经过、说话提示等，role 必须写“旁白”。
11. 凡是 role 为“旁白”的片段，dialog 必须与原文对应叙述内容保持完全一致，不得改写、增删、概括、润色或替换。
12. 旁白片段可以根据朗读节奏拆分，但拆分后的旁白 dialog 按顺序拼接后，应与原文中的对应旁白叙述完全一致。
13. 不得出现“主旁白”“冷静旁白”“悬疑旁白”“环境旁白”“动作旁白”“回忆旁白”等其他旁白角色。
14. 角色对白必须归属具体说话角色；人物心理活动必须归属对应人物；无法判断具体姓名时，使用稳定称谓，如“男主”“女主”“老人”“少年”等。
15. 同一角色名称必须前后一致；如果后文已能判断姓名，应统一使用姓名，不要在同一人物上混用“他”“男人”“男主”等称谓。
16. 非旁白角色的 dialog 必须是该角色视角下可直接朗读的内容，只保留其对白或心理独白本身。
17. 非旁白角色的 dialog 不得包含第三人称动作、神态、语气提示、说话提示或旁白叙述，例如“他冷笑道”“天子哼笑一声”“她低下头想”“沈砚沉默片刻”等。
18. 如果一句原文中包含“叙述 + 对白”，必须拆分：叙述部分归“旁白”且保持原文一致；对白部分归具体角色，并去掉外层说话提示，只保留角色实际说出的内容。
19. 示例：原文“天子哼笑一声，‘你们兄妹倒是两位一体。’”应拆为：
    [
    {"dialog":"天子哼笑一声，","role":"旁白","gender":"男","tts_control":"使用统一旁白声线，语速略慢，重音落在动作上，带出人物情绪将要转折的停顿感"},
    {"dialog":"你们兄妹倒是两位一体。","role":"明恪","gender":"男","tts_control":"带一丝凉薄的哼笑感，语调低沉，句尾轻轻下压，透出讽刺与压抑的不甘"}
    ]
20. 如果叙述片段明显是某个角色的内心想法、心理活动、主观感受或意识流，role 应归属为该角色的心理独白，例如“沈砚心理独白”“女主心理独白”，而不是写“旁白”。
21. 心理独白的 gender 必须与对应角色的 gender 保持一致。
22. 心理独白的 dialog 应转换为该角色内心视角下可直接朗读的内容，不得夹带外部叙述提示。
23. 角色对白和心理独白的 dialog 可以为了朗读流畅和逻辑清晰进行轻微顺句，但不得改变原文含义、人物态度、情绪方向、人物关系、事件顺序和关键信息。
24. 不得新增原文中不存在的剧情、动作、心理、设定、人物关系或对白。
25. 不得把原文对白改写成意思不同的新对白；如需整理对白，只能做不影响含义的轻微顺句。
26. 原文中已有的叙述、动作、环境、氛围、心理活动和对白都应根据有声书表现需要合理保留，不要随意省略关键内容。
27. 切分应服务于多角色有声书配音：当说话人、心理视角、情绪状态、场景节奏、TTS 声线或语气需要变化时，应切分为新的片段。
28. 不要把多个不同角色的对白合并到同一条 dialog 中；不同角色发言必须拆开。
29. 如果连续多句属于同一角色、同一情绪、同一语气状态，可以合并为一条；如果情绪、目的、语气或节奏发生变化，应拆分为多条。
30. 原文中已有对白，引号内的对白内容应尽量保留；确需整理时，不得改变对白含义和语气方向。
31. 非旁白角色 dialog 通常不保留外层引号，除非保留引号有助于表达原文中的引用、转述或特殊语气。
32. tts_control 用于控制 TTS 朗读语气，不限制字数，但必须具体、可执行、可表演，说明声线、语速、音量、停顿、气息、重音、节奏和情绪层次。
33. tts_control 不要只写笼统情绪词，如“开心”“难过”“生气”“复杂”“正常”“无”。必须写成细致的 TTS 语气控制说明。
34. tts_control 应根据角色和文本内容体现有声书表演差异：对白要体现人物当下语气，心理独白要体现内在波动，旁白要体现画面感和叙事节奏。
35. 旁白类 tts_control 应统一使用同一个旁白声线，但可以根据文本内容调整语速、停顿、重音、气息和叙事节奏。例如沉稳铺陈、压低声线制造悬疑、语速放慢突出回忆感、轻声带过环境细节、节奏紧凑推动动作场面等。
36. 对白类 tts_control 应明确人物语气，例如试探、克制、讽刺、压迫、慌乱、隐忍、疲惫、敷衍、强装镇定等，并说明语速、停顿、重音和尾音变化。
37. 心理独白类 tts_control 应体现内心状态，例如自我压抑、迟疑、混乱、警觉、怀疑、动摇、隐秘期待、强行说服自己等，并说明声音应更贴近内心、更低、更轻、更慢或更急促。
38. JSON 必须可解析，字符串中的引号、换行、省略号等需要正确转义。
39. 最终输出必须是一个 JSON 数组，数组外不得有任何额外文字。

错误示例:
[
{"dialog":"天子哼笑一声，“你们兄妹倒是两位一体。”","role":"明恪","gender":"男","tts_control":"低沉讽刺"}
]

错误原因:
非旁白角色 dialog 中混入了“天子哼笑一声”这种第三人称动作叙述；tts_control 过于笼统，不够具体、可执行。

正确示例:
[
{"dialog":"天子哼笑一声，","role":"旁白","gender":"男","tts_control":"使用统一旁白声线，语速略慢，动作描写处稍作停顿，重音落在“哼笑”上，带出压抑而冷淡的氛围"},
{"dialog":"你们兄妹倒是两位一体。我也曾有兄弟姐妹，却只想要我的命……今日若是他们坐在这个位子上，不知道是否会和我作出同样的决定。","role":"明恪","gender":"男","tts_control":"带有一丝刻骨的凉薄和自嘲，语调低沉，前半句带讽刺，后半句语速放慢，停顿加深，尾音轻轻下沉，透出深切的孤独感"}
]
...
1. 输出的必须是人类可直接阅读、可直接朗读的可见文字。
2. 如果原文中出现形如 `<0xE8>`、`<0xB0>`、`<0xA1>` 的字节占位符，应尽量按 UTF-8 解码为对应的可见文字。
3. 不得在最终 `dialog` 中保留 `<0x..>` 形式的乱码符号。
4. 例如原文中出现 `<0xE8><0xB0><0xA1>`，输出时应写成 `谁`，不得写成 `<0xE8><0xB0><0xA1>`。
5. 如果连续字节可以组成中文、标点、英文或数字，必须还原为对应的可见字符。
6. 如果某段乱码无法可靠解码为可见文字，应删除该乱码符号，不得输出 `<0x..>`、`�`、`□`、`<?>`、`乱码` 等占位内容。
7. JSON 字符串中应尽量直接输出中文、标点和正常文字，不要使用不可读的字节形式或乱码占位符。
8. 在不改变原文含义的前提下，必须优先保证是可朗读的自然文本。
...
async def run_stream(prompt: str, model_id: str, base_url: str, api_key: str) -> str:
    ...
    event_stream = stream(
        ...
        ModelOptions(api_options=OpenRouterCompletionsStreamOptions(reasoning={"effort": "none"}, verbosity="low"),),
        # ModelOptions(api_options=OpenRouterCompletionsStreamOptions(reasoning={"effort": "low"}, verbosity="low"),),
        # ModelOptions(),
    )

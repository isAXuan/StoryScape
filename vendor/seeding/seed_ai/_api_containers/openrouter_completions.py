"""OpenRouter 专用 Chat Completions 协议提供商适配器实现。

基于 OpenAI 旧版通用 Chat Completions 协议，针对 OpenRouter 进行定制：
- 添加 OpenRouter 必需的 HTTP Referrer 和 X-Title headers
- 使用 OpenRouter 特有的路由选项
- 适配 OpenRouter 的响应格式
"""
...
def stream_openrouter_completions(
    model: Model,
    context: Context,
    stream_options: StreamOptions,
    model_options: ModelOptions | None = None,
) -> AssistantMessageEventStream:
    """全面接管并控制 OpenRouter 的 completions 请求，封装并向外围丢出多端统一时序流管实例。"""
